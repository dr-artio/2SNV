...
//    }
    ...
    while (seeds.size < k && !seeds.forall(_.convergen)) {
      ...
      for (t <- seeds.filterNot(_.convergen)) {
        ...
        val newCenter = t.getSecondGenotype
        val secondGenotype: Genotype = if (newCenter != None) newCenter.asInstanceOf[Genotype] else null
...
//        }
        ...
        if (secondGenotype != null ) {
          ...
          log("New seed size: %f".format(secondGenotype.size))
          ...
          if (newSeed.size > 1) {
            ...
            if (seed.size < 2 * KGEM.threshold) seed.convergen = true
            seeds += seed
//            for (s <- seeds) {
